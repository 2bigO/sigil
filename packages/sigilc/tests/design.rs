...
fn required_authored_inventory_cannot_disappear_from_empty_model_output() {
    ...
    let empty = kernel::design(&[], &units, Limits::default()).unwrap();
    ...
    let complete = kernel::design(&prohibition, &units, Limits::default()).unwrap();
    ...
    let erased =
        kernel::design(&facts(":U s:required false ."), &units, Limits::default()).unwrap();
...
fn design_states_preserve_capability_ownership_and_contradiction_rules() {
    for (body, expected) in [
        ...
        assert_eq!(
            kernel::design(&facts(body), &[], Limits::default())
                .unwrap()
                .state,
            expected,
            "{body}"
...
fn fixed_lowering_separates_descriptive_edges_from_code_obligations() {
    ...
    let output = kernel::design(&input, &[], Limits::default()).unwrap();
    ...
    let implementation = kernel::saturate(&input, Limits::default()).unwrap();
