...
7. Brownfield reconstruction and externally informed additions are shown as exact proposed Facets before the agent edits Sigil.
...
The agent presents the exact Facets, target files, locations, imports, evidence, conflicts, and uncertainty, then waits for explicit approval.
...
After approval, a placement-only move or split that preserves approved Facets may proceed without another semantic review gate.
Required import-path updates are placement-only.
Any added, removed, or changed Facet creates another review gate.
...
3. Produce no more than twenty inspectable source candidates for each selected Facet.
